...
const LANES: { id: Lane; label: string; icon: string }[] = [
  { id: "reis", label: "Reis", icon: "👑" },
  { id: "profetas", label: "Profetas", icon: "📖" },
  { id: "sacerdotes", label: "Sacerdotes/Líderes", icon: "🕎" },
...
const ParallelsView = ({ onNavigate }: Props) => {
  ...
  const items = useMemo(() => {
    const byLane: Record<Lane, Array<{ id: string; name: string; icon: string; year: number; kind: "character" | "event" }>> = {
      reis: [], profetas: [], sacerdotes: [], eventos: [],
    };
    for (const c of CHARACTERS) {
      if (c.tags.includes("rei")) byLane.reis.push({ id: c.id, name: c.name, icon: c.icon, year: c.year, kind: "character" });
      if (c.tags.includes("profeta")) byLane.profetas.push({ id: c.id, name: c.name, icon: c.icon, year: c.year, kind: "character" });
      if (c.tags.includes("sacerdote") || c.tags.includes("juiz") || c.tags.includes("lider"))
        byLane.sacerdotes.push({ id: c.id, name: c.name, icon: c.icon, year: c.year, kind: "character" });
    }
    for (const e of EVENTS) {
      byLane.eventos.push({ id: e.id, name: e.name, icon: e.icon, year: e.year, kind: "event" });
    }
    return byLane;
  }, []);
        ...
        <div className="relative" style={{ width: totalWidth }}>
          ...
          <div className="relative h-6 mb-2">
            {PERIODS.filter((p) => p.startYear >= START && p.endYear <= END).map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  className="absolute top-0 h-6 rounded-lg flex items-center px-2 overflow-hidden bg-dark-card border border-dark-card-hover"
                  style={{
                    left: x1,
          ...
          {LANES.filter((l) => activeLanes.includes(l.id)).map((l) => (
            <div key={l.id} className="relative h-14 mt-2">
              <div
                className="absolute inset-x-0 top-1/2 h-px bg-dark-card-hover"
              />
              <div
                className="absolute left-0 top-1/2 -translate-y-1/2 z-10 text-[10px] font-bold px-2 py-0.5 rounded bg-primary text-primary-foreground"
              ...
              {items[l.id].map((it) => {
                ...
                return (
                  ...
                  >
                    <div
                      className="w-8 h-8 rounded-full flex items-center justify-center shadow-lg text-sm transition-transform group-hover:scale-125 bg-dark-card border-2 border-primary"
                    >
                      {it.icon}
                    </div>
                    <span
                      className="absolute left-1/2 -translate-x-1/2 top-9 whitespace-nowrap text-[9px] font-semibold px-1 rounded opacity-0 group-hover:opacity-100 group-focus:opacity-100 transition-opacity pointer-events-none bg-dark-card text-dark-text border border-dark-card-hover"
                    >
                      {it.name}
                    </span>
                  </button>
