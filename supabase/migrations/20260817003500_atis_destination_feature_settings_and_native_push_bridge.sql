create table if not exists public.atis_destination_feature_settings (
  id uuid primary key default gen_random_uuid(),
  destination_type text not null check (destination_type in ('contact','individual','group')),
  contact_id uuid references public.atis_contacts(id) on delete cascade,
  individual_id uuid references public.atis_individuals(id) on delete cascade,
  group_id uuid references public.atis_groups(id) on delete cascade,
  feature_kind text not null check (feature_kind in ('ai','push')),
  feature_key text not null,
  enabled boolean not null default false,
  schedule_mode text not null default 'system' check (schedule_mode in ('system','custom_time')),
  custom_time time without time zone,
  timezone text not null default 'America/Fortaleza',
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint atis_destination_feature_exact_target_chk check (
    (destination_type = 'contact' and contact_id is not null and individual_id is null and group_id is null)
    or (destination_type = 'individual' and contact_id is null and individual_id is not null and group_id is null)
    or (destination_type = 'group' and contact_id is null and individual_id is null and group_id is not null)
  ),
  constraint atis_destination_feature_key_chk check (
    (feature_kind = 'ai' and feature_key in ('ask_bible','exegetai','chapter_summary','word_meaning','connections','timeline','devotional'))
    or (feature_kind = 'push' and feature_key in ('general','daily-verse','motivational','culto-reminder'))
  ),
  constraint atis_destination_feature_schedule_chk check (
    (feature_kind = 'ai' and schedule_mode = 'system' and custom_time is null)
    or (feature_kind = 'push' and schedule_mode = 'system' and custom_time is null)
    or (feature_kind = 'push' and schedule_mode = 'custom_time' and custom_time is not null)
  )
);

create unique index if not exists atis_destination_feature_contact_uidx
  on public.atis_destination_feature_settings(contact_id, feature_kind, feature_key)
  where contact_id is not null;
create unique index if not exists atis_destination_feature_individual_uidx
  on public.atis_destination_feature_settings(individual_id, feature_kind, feature_key)
  where individual_id is not null;
create unique index if not exists atis_destination_feature_group_uidx
  on public.atis_destination_feature_settings(group_id, feature_kind, feature_key)
  where group_id is not null;
create index if not exists atis_destination_feature_push_lookup_idx
  on public.atis_destination_feature_settings(feature_kind, feature_key, enabled)
  where feature_kind = 'push' and enabled = true;

alter table public.atis_destination_feature_settings enable row level security;
revoke all on table public.atis_destination_feature_settings from anon, authenticated;
grant all on table public.atis_destination_feature_settings to service_role;

drop trigger if exists set_atis_destination_feature_settings_updated_at on public.atis_destination_feature_settings;
create trigger set_atis_destination_feature_settings_updated_at
before update on public.atis_destination_feature_settings
...
begin
  ...
  v_event_key := coalesce(nullif(btrim(_event_key), ''), encode(digest(coalesce(_push_type,'') || '|' || coalesce(_title,'') || '|' || coalesce(_body,'') || '|' || clock_timestamp()::text, 'sha256'), 'hex'));
