alter table public.atis_destination_feature_settings
  drop constraint if exists atis_destination_feature_settings_feature_kind_check,
  drop constraint if exists atis_destination_feature_settings_schedule_mode_check,
  drop constraint if exists atis_destination_feature_key_chk,
  drop constraint if exists atis_destination_feature_schedule_chk;

alter table public.atis_destination_feature_settings
  add constraint atis_destination_feature_settings_feature_kind_check
    check (feature_kind in ('ai','push','automation')),
  add constraint atis_destination_feature_settings_schedule_mode_check
    check (schedule_mode in ('system','instant','custom_time')),
  add constraint atis_destination_feature_key_chk
    check (
      (feature_kind = 'ai' and feature_key in ('ask_bible','exegetai','chapter_summary','word_meaning','connections','timeline','devotional'))
      or
      (feature_kind = 'push' and feature_key in ('general','daily-verse','motivational','culto-reminder'))
      or
      (feature_kind = 'automation' and feature_key in ('birthdays'))
    ),
  add constraint atis_destination_feature_schedule_chk
    check (
      (feature_kind = 'ai' and schedule_mode = 'system' and custom_time is null)
      or
      (feature_kind in ('push','automation') and schedule_mode in ('system','instant') and custom_time is null)
      or
      (feature_kind in ('push','automation') and schedule_mode = 'custom_time' and custom_time is not null)
    );
...
set value = jsonb_build_object(
      'mode', 'per_destination',
      'timezone', coalesce(nullif(value->>'timezone',''), 'America/Fortaleza'),
      'message_template', value->'message_template',
      'system_schedule', 'instant'
    ),
    description = 'Birthday content settings. Delivery activation and schedule are configured per destination in atis_destination_feature_settings.',
    updated_at = now()
where key = 'birthdays';
...
begin
  ...
  v_event_key := coalesce(
    nullif(btrim(_event_key), ''),
    md5(coalesce(_push_type,'') || '|' || coalesce(_title,'') || '|' || coalesce(_body,'') || '|' || clock_timestamp()::text)
  );
  v_content := '*' || btrim(_title) || '*' || E'\n' || btrim(_body)
    || case when nullif(btrim(coalesce(_url,'')), '') is not null then E'\n\n' || btrim(_url) else '' end;
  ...
  loop
    ...
    if r.destination_type = 'contact' then
      insert into public.atis_message_targets(
        ...
        jsonb_build_object('native_push_type',_push_type,'schedule_mode',r.schedule_mode)
      );
