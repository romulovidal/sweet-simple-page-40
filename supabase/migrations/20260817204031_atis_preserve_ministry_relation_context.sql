-- Keep grounded Culto + Bible + Worship relation answers inside the existing
-- short-lived ministry conversation session. No new table or public privilege.
...
begin
  ...
  if candidate_reference is not null
    ...
    merged_memory := jsonb_set(merged_memory, '{last_bible_reference}', to_jsonb(candidate_reference), true);
    merged_memory := jsonb_set(merged_memory, '{last_bible_reference_at}', to_jsonb(now()::text), true);
  end if;

  if candidate_reference is not null
     and candidate_reference like 'ctx:%'
     and route_name = any (array['culto_info', 'canticos_info', 'harpa_lookup', 'ministry_relation']) then
    merged_memory := jsonb_set(merged_memory, '{last_ministry_reference}', to_jsonb(candidate_reference), true);
    merged_memory := jsonb_set(merged_memory, '{last_ministry_reference_at}', to_jsonb(now()::text), true);
  end if;
